...
SHUVR_BIN="${SHUVR_BIN:-$ROOT/target/debug/shuvr}"
BASE="${BASE:-$(mktemp -d /tmp/shuvr-handoff-smoke.XXXXXX)}"
...
cleanup() {
  ...
  for session in "${sessions[@]}"; do
    run_shuvr "$session" server stop >/dev/null 2>&1 || true
  done
...
run_shuvr() {
  ...
  env -u SHUVR_SOCKET_PATH \
    -u SHUVR_CLIENT_SOCKET_PATH \
    -u SHUVR_SESSION \
    XDG_CONFIG_HOME="$CONFIG_HOME" \
    XDG_RUNTIME_DIR="$RUNTIME_DIR" \
    XDG_STATE_HOME="$STATE_DIR" \
    "$SHUVR_BIN" --session "$session" "$@"
}

session_dir() {
  local session="$1"
  if [[ "$session" == "default" ]]; then
    printf '%s/shuvr-dev' "$CONFIG_HOME"
  else
    printf '%s/shuvr-dev/sessions/%s' "$CONFIG_HOME" "$session"
  fi
}

api_socket() {
  printf '%s/shuvr.sock' "$(session_dir "$1")"
}

client_socket() {
  printf '%s/shuvr-client.sock' "$(session_dir "$1")"
}

assert_smoke_socket() {
  local socket="$1"
  case "$socket" in
    "$CONFIG_HOME"/shuvr-dev/shuvr.sock | "$CONFIG_HOME"/shuvr-dev/sessions/*/shuvr.sock)
      ;;
    *)
...
echo "using shuvr: $SHUVR_BIN"
echo "smoke base: $BASE"

cargo build --locked --manifest-path "$ROOT/Cargo.toml" >/dev/null
mkdir -p "$CONFIG_HOME/shuvr-dev" "$RUNTIME_DIR" "$STATE_DIR"
printf 'onboarding = false\n' > "$CONFIG_HOME/shuvr-dev/config.toml"

for session in "${sessions[@]}"; do
  echo "starting smoke session $session at $(api_socket "$session")"
  run_shuvr "$session" server >/dev/null 2>&1 &
  server_pids+=("$!")
