---
title: Socket API
description: Control a running Shuvr server from scripts, tools, and coding agents.
---

Shuvr exposes a local socket API for scripts and agents that need to inspect or control a running session.
...
| Layer | Use it for |
| --- | --- |
| Agent skill | Teaching a coding agent how to use Shuvr from inside a pane. |
...
```bash
shuvr workspace create --cwd ~/project --label api
```

Create a tab:

```bash
shuvr tab create --label logs
```

Split a pane and run a command:

```bash
shuvr pane split 1-1 --direction right
shuvr pane run 1-2 "npm test"
```

Wait for an agent:

```bash
shuvr wait agent-status 1-1 --status done
```

Read pane output:

```bash
shuvr pane read 1-2 --source recent --lines 50
```
...
Some CLI commands are conveniences around these methods. For example, `shuvr agent wait` resolves an agent target and then subscribes to pane agent state events.

Worktree methods manage Git checkouts as Shuvr workspaces. `worktree.create` creates a checkout and returns the new `workspace`, `tab`, `root_pane`, and `worktree` records. `worktree.open` opens an existing checkout or returns the already-open workspace. `worktree.remove` runs `git worktree remove` against a linked child workspace and never deletes the branch.
...
The default socket lives under your Shuvr config directory.

Named sessions have separate sockets:

```text
~/.config/shuvr/shuvr.sock
~/.config/shuvr/sessions/<name>/shuvr.sock
```
...
2. `SHUVR_SOCKET_PATH`
3. `SHUVR_SESSION=<name>`
4. default session socket

Use `SHUVR_SOCKET_PATH` only for low-level overrides.
...
Official integrations can also report a native session reference. `pane.get`, `pane.list`, `agent.get`, and `agent.list` expose a read-only `agent_session` object when Shuvr has a stored native session reference:
...
`pane.get`, `pane.list`, `agent.get`, and `agent.list` also expose `foreground_cwd` when Shuvr can resolve the cwd of the process currently controlling the pane PTY. The existing `cwd` field remains the pane/workspace cwd used for labels, follow-cwd behavior, and restored session state.

Use `pane.report_metadata` when a user hook wants to customize presentation without taking over lifecycle state from a Shuvr integration.
...
```json
{
  ...
  "params": {
    ...
    "agent": "claude",
    "applies_to_source": "shuvr:claude",
...
```bash
shuvr pane read 1-1 --source visible --lines 80
shuvr pane read 1-1 --source recent --lines 120
shuvr pane read 1-1 --source recent-unwrapped --lines 120
```
...
```bash
shuvr wait agent-status 1-1 --status done
shuvr wait agent-status 1-1 --status blocked
```
...
Shuvr has a protocol version for client/server compatibility. Protocol changes are reviewed with release compatibility in mind.

Check the server protocol with `ping` or `shuvr status` before depending on new behavior. Handle unknown fields gracefully.
