---
title: Configuration
description: Configure Shuvr keybindings, themes, sidebar behavior, notifications, and advanced options.
---

Shuvr works without a config file. Add one when you want custom keys, themes, sidebar settings, notifications, or advanced behavior.

## Config file

Shuvr reads config from:

```text
~/.config/shuvr/config.toml
```

Print the full default config:

```bash
shuvr --default-config
```

Save it as your config if you want a complete starting point:

```bash
shuvr --default-config > ~/.config/shuvr/config.toml
```

If a config value is invalid, Shuvr falls back to a safe default and shows a startup warning.

Shuvr shows first-run setup when `onboarding` is missing or true. Continuing from onboarding writes `onboarding = false` and opens settings on the integrations tab. Set it when you want to skip that flow after setup.
...
You can also open the global menu in Shuvr and choose `reload config`.

Reload applies most UI settings without restarting panes. Startup-only settings still need a restart.

## Terminal defaults

Set the executable Shuvr uses for newly created interactive panes:
...
When unset or empty, Shuvr uses `$SHELL`, then `/bin/sh`. This is an executable name or path, not a shell command line. Existing panes keep their current shell until they are recreated. Command panes still run through `/bin/sh -c`; detached custom command keybindings use Shuvr's existing `/bin/sh -lc` path.

Set how Shuvr starts newly created interactive pane shells:
...
`new_cwd = "follow"` keeps the default behavior and inherits the source pane or workspace. Use `"home"` to always start in `$HOME`, `"current"` to use Shuvr's process directory, or a fixed path such as `"~/Projects"`. Explicit `--cwd` values from the CLI or socket API still take precedence.

## Worktrees

Set the root directory Shuvr uses for Git worktree checkouts created from the sidebar:
...
Shuvr creates checkouts under `<directory>/<repo>/<branch-slug>`. For sibling-style checkouts, set this to a directory such as `~/Projects/shuvr-worktrees`. Relative values are resolved to an absolute path when the app applies the config.

Worktree actions are available from Git workspace rows. `New worktree` creates a branch and checkout, opens it as a new Shuvr workspace, and groups it under the source workspace. `Open worktree...` lists existing Git worktree checkouts for that repo; choosing an already-open checkout focuses it, and choosing a closed checkout opens it in the same group.

Grouped worktrees still behave like normal Shuvr workspaces: they can be focused, renamed, closed, and contain their own tabs and panes. The parent row is the original workspace. Closing the parent row closes the whole Shuvr group, but it does not delete checkout folders or branches.

Deleting a worktree checkout is explicit. Use `Delete worktree checkout...` on a grouped child workspace to run `git worktree remove`. Shuvr first asks Git to remove safely. If Git refuses because the checkout has modified or untracked files, Shuvr asks again before running the forced remove. Branches are not deleted.

## Keybindings

Shuvr has a prefix mode similar to tmux. The default prefix is `ctrl+b`. Keybinding strings are explicit: `prefix+n` means press the configured prefix and then `n`; `ctrl+alt+n` is a direct terminal-mode shortcut.
...
If you have old custom keybindings and want the new defaults, run `shuvr config reset-keys`. Shuvr backs up `config.toml`, removes `[keys]` and `[[keys.command]]`, and uses built-in v2 defaults after restart or `shuvr server reload-config`.
...
Custom commands receive `SHUVR_SOCKET_PATH`, `SHUVR_BIN_PATH`, `SHUVR_ACTIVE_WORKSPACE_ID`, `SHUVR_ACTIVE_TAB_ID`, `SHUVR_ACTIVE_PANE_ID`, and `SHUVR_ACTIVE_PANE_CWD` when those values are available. Shell commands run from the focused pane's working directory when Shuvr can detect it.
...
Use `terminal` when you want Shuvr UI colors to follow your host terminal's ANSI palette.
...
The sidebar is the main Shuvr dashboard. It shows workspaces, tabs, panes, and agent state.
...
`mobile_width_threshold` controls the terminal width at or below which Shuvr uses the mobile single-column layout. The default is 64 columns; increase it for foldables, tablets, or wide phone terminals.
...
Set `mouse_capture = false` if you want your terminal to handle normal clicks, such as command-clicking URLs. With mouse capture enabled, Ctrl-click opens pane links when your terminal sends that modified click to Shuvr; use Shift-Ctrl-click on Linux or Shift-Cmd-click on macOS for the terminal-native bypass path.

Set `redraw_on_focus_gained = false` to avoid the visible full-screen refresh when switching back to Shuvr. The default is `true` because a full redraw recovers from rare stale or dirty host terminal surfaces.
...
Shuvr can show popup notifications when agents finish or need input.
...
`delivery = "shuvr"` shows a top-right toast inside the Shuvr UI. Click the toast, or bind `keys.open_notification_target`, to focus the target workspace, tab, and pane.

`delivery = "terminal"` asks the outer terminal to show a desktop notification. Shuvr sends terminal notification escape sequences for Ghostty, iTerm2, Kitty, and WezTerm. This is useful over SSH because the local terminal owns the notification.

`delivery = "system"` asks the local operating system directly. On macOS, Shuvr uses `terminal-notifier` when available, then falls back to `/usr/bin/osascript`. `terminal-notifier` can activate the hosting terminal when you click the notification. On Linux, Shuvr uses `notify-send` and requires `DISPLAY` or `WAYLAND_DISPLAY`.

Popup notifications are for background attention. Shuvr suppresses popups for the active tab.

## Sound

Sound notifications are enabled by default and are played by the local Shuvr client.
...
Shuvr plays a done sound when an agent finishes and an attention sound when an agent needs input. Set `enabled = false` on shared machines or remote servers unless you explicitly want audio.

On macOS, Shuvr uses `afplay`. On Linux, Shuvr tries mp3-capable players in order: `paplay`, `pw-play`, `ffplay`, `mpg123`, then `mpv`. If no player is available, sound playback is skipped and Shuvr logs a warning.
...
Pane screen history is off by default. Pane output can include secrets, tokens, prompts, and command output, so enable it only when you want Shuvr to save recent pane contents across full server restarts:
...
When enabled, Shuvr stores saved pane history in `session-history.json` next to `session.json`.
...
Shuvr normally protects you from launching Shuvr inside Shuvr.
...
Shuvr can restart supported agent panes in their native conversation sessions after a Shuvr server restart.
...
When enabled, Shuvr only resumes panes that reported a native session reference through an official Shuvr integration. Supported resume targets are Claude Code, Codex, Pi, Hermes Agent, and OpenCode. Unsupported, missing, invalid, duplicated, or stale session references restore as a normal shell in the saved pane directory.

Session references are stored in the local Shuvr session snapshot. They are not shown in normal pane, agent, status, or event output.
...
| Variable | Purpose |
| --- | --- |
| `SHUVR_CONFIG_PATH` | Override the config file path. |
| `SHUVR_SESSION` | Select a named session for CLI commands. |
| `SHUVR_SOCKET_PATH` | Low-level socket path override. |
| `SHUVR_LOG` | Set log filtering, for example `SHUVR_LOG=shuvr=debug`. |
| `SHUVR_DISABLE_SOUND` | Disable sound playback even when `[ui.sound] enabled = true`. |
...
```text
~/.config/shuvr/shuvr.log
~/.config/shuvr/shuvr-client.log
~/.config/shuvr/shuvr-server.log
```
