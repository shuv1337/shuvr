...
use support::{
    ...
    read_server_message, register_runtime_dir, register_spawned_shuvr_pid,
    unregister_spawned_shuvr_pid, wait_for_file, wait_for_message_variant, wait_for_socket,
    wait_until,
};
...
fn server_unreachable_shows_clear_error() {
    ...
    let output = std::process::Command::new(env!("CARGO_BIN_EXE_shuvr"))
        .arg("client")
        .env("SHUVR_DISABLE_SOUND", "1")
    ...
    assert!(
        stderr.contains("Is shuvr server running?"),
