...
use support::{
    cleanup_test_base, register_runtime_dir, register_spawned_shuvr_pid,
    unregister_spawned_shuvr_pid,
};
...
fn unique_test_dir() -> PathBuf {
    let nanos = SystemTime::now()
        ...
        .unwrap_or(0);
    PathBuf::from(format!(
        "/tmp/shuvr-autodetect-test-{}-{nanos}",
        std::process::id()
    ))
}
...
impl Drop for SpawnedShuvr {
    fn drop(&mut self) {
        ...
        if let Some(pid) = pid {
            ...
            unregister_spawned_shuvr_pid(Some(pid));
        }
    }
}

fn cleanup_spawned_shuvr(spawned: SpawnedShuvr, base: PathBuf) {
    drop(spawned);
...
fn spawn_server(
    ...
    cmd.env("SHUVR_SOCKET_PATH", api_socket_path);
    cmd.env_remove("SHUVR_CLIENT_SOCKET_PATH");
    cmd.env("SHELL", "/bin/sh");
    cmd.env_remove("SHUVR_ENV");
...
/// Spawn `shuvr` (no subcommand) — the auto-detect launch path.
fn spawn_shuvr_auto(
    config_home: &Path,
    runtime_dir: &Path,
    api_socket_path: &Path,
...
/// Spawn `shuvr --no-session` — the monolithic escape hatch.
fn spawn_shuvr_no_session(
    config_home: &Path,
    runtime_dir: &Path,
    api_socket_path: &Path,
) -> SpawnedShuvr {
...
fn run_cli(socket_path: &Path, args: &[&str]) -> std::process::Output {
    let mut command = Command::new(env!("CARGO_BIN_EXE_shuvr"));
    command.args(args);
    command.env("SHUVR_SOCKET_PATH", socket_path);
...
/// Running `shuvr` with no server present starts a server
...
fn auto_detect_no_server_spawns_server_and_attaches() {
    ...
    // Run `shuvr` (no subcommand) — should auto-detect, spawn server, attach as client.
    let shuvr = spawn_shuvr_auto(&config_home, &runtime_dir, &api_socket, &client_socket);
    ...
    let client_pid = shuvr.child.process_id().expect("client should have PID");
    assert!(
        process_exists(client_pid),
        "client process should be running"
    );

    cleanup_spawned_shuvr(shuvr, base);
}

/// Running `shuvr` with a server already running attaches
...
fn auto_detect_server_running_attaches_directly() {
    ...
    // Run `shuvr` (no subcommand) — should detect the running server and attach.
    let client = spawn_shuvr_auto(&config_home, &runtime_dir, &api_socket, &client_socket);
    ...
    cleanup_spawned_shuvr(client, PathBuf::from("/nonexistent"));
    cleanup_spawned_shuvr(server, base);
}

/// Socket path resolution is consistent between server and client.
/// Both derive the client socket from the `SHUVR_SOCKET_PATH` override,
...
fn auto_detect_socket_path_consistency() {
    ...
    // Run `shuvr` with custom socket paths.
    let shuvr = spawn_shuvr_auto(&config_home, &runtime_dir, &api_socket, &client_socket);
...
/// `shuvr --no-session` bypasses server/client and runs
...
fn no_session_flag_runs_monolithically() {
    ...
    // Run `shuvr --no-session` — monolithic mode, no server/client.
    let shuvr = spawn_shuvr_no_session(&config_home, &runtime_dir, &api_socket);
    ...
    // there is only one shuvr process.
    let client_pid = shuvr.child.process_id().expect("should have PID");
    assert!(
        process_exists(client_pid),
...
fn cli_subcommands_work_through_server() {
    ...
    // Test `shuvr workspace list` through the server's API socket.
    ...
    // Test `shuvr pane list` through the server's API socket.
...
/// persists after the client exits, and a new `shuvr` can reattach.
#[test]
fn auto_detect_server_persists_and_reattaches() {
    ...
    // Run `shuvr` — auto-detect spawns server + attaches client.
    let mut client1 = spawn_shuvr_auto(&config_home, &runtime_dir, &api_socket, &client_socket);
    wait_for_socket(&api_socket, Duration::from_secs(10));
    wait_for_socket(&client_socket, Duration::from_secs(10));
    ...
    // Run `shuvr` again — should detect the running server and reattach.
    let client2 = spawn_shuvr_auto(&config_home, &runtime_dir, &api_socket, &client_socket);
    thread::sleep(Duration::from_millis(500));
    ...
    cleanup_spawned_shuvr(client2, PathBuf::from("/nonexistent"));
    cleanup_test_base(&base);
}
...
fn auto_detect_default_socket_path_from_config_dir() {
    ...
    // Don't set SHUVR_SOCKET_PATH or SHUVR_CLIENT_SOCKET_PATH.
    ...
    let api_socket = config_home.join(app_dir_name).join("shuvr.sock");
    let client_socket = config_home.join(app_dir_name).join("shuvr-client.sock");
...
fn auto_detect_writes_client_and_server_logs_to_separate_files() {
    ...
    let spawned = spawn_shuvr_auto(&config_home, &runtime_dir, &api_socket, &client_socket);
    wait_for_socket(&api_socket, Duration::from_secs(10));
    wait_for_socket(&client_socket, Duration::from_secs(10));
    ...
    let client_log = log_dir.join("shuvr-client.log");
    let server_log = log_dir.join("shuvr-server.log");
    let monolith_log = log_dir.join("shuvr.log");
    ...
    assert!(
        ...
        "persistent client logs should not land in shuvr.log: {monolith_content}"
    );

    cleanup_spawned_shuvr(spawned, base);
}
...
fn no_session_writes_startup_logs_to_monolith_file() {
    ...
    let spawned = spawn_shuvr_no_session(&config_home, &runtime_dir, &api_socket);
    wait_for_socket(&api_socket, Duration::from_secs(10));
...
fn auto_detect_respects_nested_guard_before_auto_attach() {
    ...
    let output = Command::new(env!("CARGO_BIN_EXE_shuvr"))
        .env("XDG_CONFIG_HOME", &config_home)
        .env("XDG_RUNTIME_DIR", &runtime_dir)
        .env("SHUVR_SOCKET_PATH", &api_socket)
        .env_remove("SHUVR_CLIENT_SOCKET_PATH")
        .env("SHUVR_ENV", "1")
        .output()
        .unwrap();
    ...
    assert!(
        stderr.contains("nested shuvr is disabled by default"),
