...
fn unique_test_dir() -> PathBuf {
    let nanos = SystemTime::now()
        ...
        .unwrap_or(0);
    PathBuf::from(format!(
        "/tmp/shuvr-multi-client-test-{}-{nanos}",
        std::process::id()
    ))
}
...
fn spawn_server(config_home: &Path, runtime_dir: &Path, api_socket_path: &Path) -> SpawnedShuvr {
...
fn server_log_path(config_home: &Path) -> PathBuf {
    ...
    config_home.join(app_dir).join("shuvr-server.log")
}
