...
fn create_committed_repo(path: &Path) {
    ...
    run_git(path, &["config", "user.email", "shuvr@example.invalid"]);
    run_git(path, &["config", "user.name", "Shuvr Test"]);
...
fn spawn_shuvr(config_home: &Path, runtime_dir: &Path, socket_path: &Path) -> SpawnedShuvr {
    spawn_shuvr_with_config(
        config_home,
        runtime_dir,
        socket_path,
        None,
...
fn spawn_shuvr_with_pane_history(
    config_home: &Path,
    runtime_dir: &Path,
    socket_path: &Path,
) -> SpawnedShuvr {
...
fn spawn_named_server(
    ...
    command
        ...
        .env_remove("SHUVR_SOCKET_PATH")
        .env_remove("SHUVR_CLIENT_SOCKET_PATH")
        .env_remove("SHUVR_ENV")
        .stdin(std::process::Stdio::null())
        .stdout(std::process::Stdio::null())
        .stderr(std::process::Stdio::null());

    let child = command.spawn().unwrap();
    register_spawned_shuvr_pid(Some(child.id()));
...
fn run_named_cli_with_socket_override(
    ...
    if let Some(socket_override) = socket_override {
        command.env("SHUVR_SOCKET_PATH", socket_override);
    } else {
        command.env_remove("SHUVR_SOCKET_PATH");
    }
...
fn spawn_shuvr_with_config(
    ...
    cmd.env("SHUVR_SOCKET_PATH", socket_path);
...
fn parse_cli_json_output(args: &[&str], output: std::process::Output) -> serde_json::Value {
    ...
    serde_json::from_slice(&output.stdout).unwrap_or_else(|err| {
        panic!(
            "failed to parse JSON response for `shuvr {}`: {}\nstdout: {}\nstderr: {}",
            args.join(" "),
            err,
            String::from_utf8_lossy(&output.stdout),
            String::from_utf8_lossy(&output.stderr)
...
fn run_claude_hook(action: &str, hook_input: &str) -> Option<serde_json::Value> {
    run_shell_hook(
        "src/integration/assets/claude/shuvr-agent-state.sh",
        &[action],
        hook_input,
    )
}
...
fn run_codex_hook(action: &str, hook_input: &str) -> Option<serde_json::Value> {
    run_shell_hook(
        "src/integration/assets/codex/shuvr-agent-state.sh",
        &[action],
        hook_input,
    )
}
...
fn run_shell_hook(asset_path: &str, args: &[&str], hook_input: &str) -> Option<serde_json::Value> {
    let base = unique_test_dir();
    fs::create_dir_all(&base).unwrap();
    let socket_path = base.join("shuvr.sock");
    ...
    let mut child = Command::new("bash")
        ...
        .args(args)
        .env("SHUVR_ENV", "1")
        .env("SHUVR_SOCKET_PATH", &socket_path)
        .env("SHUVR_PANE_ID", "p_test")
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        .spawn()
...
fn pane_report_metadata_sends_presentation_request() {
    ...
    assert_eq!(request["params"]["applies_to_source"], "shuvr:claude");
...
fn help_commands_exit_successfully() {
    ...
    for args in help_cases {
        ...
        assert!(
            output.status.success(),
            "shuvr {} failed: status={:?} stdout={} stderr={}",
            args.join(" "),
            output.status.code(),
            String::from_utf8_lossy(&output.stdout),
            String::from_utf8_lossy(&output.stderr)
...
fn named_sessions_use_separate_servers_and_workspace_state() {
    ...
    assert!(alpha_session["socket_path"]
        .as_str()
        .unwrap()
        .ends_with("/sessions/alpha/shuvr.sock"));
...
fn integration_commands_run_locally_when_server_is_missing() {
    ...
    let expected_extension = extensions_dir.join("shuvr-agent-state.ts");
    assert!(
        !expected_extension.exists(),
        "test setup should start without extension file"
    );

    let workspace_list = Command::new(env!("CARGO_BIN_EXE_shuvr"))
        .args(["workspace", "list"])
        .env("SHUVR_SOCKET_PATH", &missing_socket)
        .env("HOME", &home_dir)
        .output()
        .unwrap();
    assert_eq!(workspace_list.status.code(), Some(1));

    let integration_install = Command::new(env!("CARGO_BIN_EXE_shuvr"))
    ...
    let integration_status = Command::new(env!("CARGO_BIN_EXE_shuvr"))
    ...
    let integration_uninstall = Command::new(env!("CARGO_BIN_EXE_shuvr"))
...
fn integration_status_outdated_only_prints_action_for_legacy_install() {
    ...
    fs::create_dir_all(&extensions_dir).unwrap();
    fs::write(
        extensions_dir.join("shuvr-agent-state.ts"),
        "// legacy shuvr integration\n",
    )
    .unwrap();
    ...
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(stderr.contains("installed shuvr integrations need updating"));
    assert!(stderr.contains("shuvr integration install pi"));
...
fn status_commands_report_client_and_server_versions() {
    ...
    let config_home = base.join("config");
    let runtime_dir = base.join("runtime");
    let socket_path = runtime_dir.join("shuvr.sock");

    let shuvr = spawn_shuvr(&config_home, &runtime_dir, &socket_path);
...
fn server_stop_command_shuts_down_running_server() {
    ...
    let mut shuvr = spawn_shuvr(&config_home, &runtime_dir, &socket_path);
    wait_for_socket(&socket_path, Duration::from_secs(5));
    ...
    let pid = shuvr.child.process_id();
    let exit_status = shuvr.child.wait().unwrap();
...
fn server_stop_then_restart_restores_pane_history() {
    ...
    let mut shuvr = spawn_shuvr_with_pane_history(&config_home, &runtime_dir, &socket_path);
    wait_for_socket(&socket_path, Duration::from_secs(5));
    wait_for_socket(&client_socket, Duration::from_secs(5));
    ...
    let restarted = spawn_shuvr_with_pane_history(&config_home, &runtime_dir, &socket_path);
    wait_for_socket(&socket_path, Duration::from_secs(5));
    wait_for_socket(&client_socket, Duration::from_secs(5));
    ...
    cleanup_spawned_shuvr(restarted, base);
}
...
fn worktree_cli_rejects_local_argument_errors_before_socket_use() {
    ...
    for args in cases {
        ...
        assert_eq!(
            output.status.code(),
            Some(2),
            "shuvr {} should fail as local parse error; stdout={} stderr={}",
            args.join(" "),
            String::from_utf8_lossy(&output.stdout),
            String::from_utf8_lossy(&output.stderr)
        );
...
#[test]
fn pane_shell_gets_shuvr_socket_and_pane_env() {
    let base = unique_test_dir();
    let config_home = base.join("config");
    let runtime_dir = base.join("runtime");
    let socket_path = runtime_dir.join("shuvr.sock");
    ...
    let ran = run_cli(
        ...
        &[
            ...
            "1-1",
            &format!(
                "printf '%s\\n%s\\n' \"$SHUVR_SOCKET_PATH\" \"$SHUVR_PANE_ID\" > {}",
                env_capture.display()
            ),
        ],
    );
...
fn wait_agent_status_exits_when_idle_status_matches() {
    ...
    let shuvr = spawn_shuvr_with_path(
        &config_home,
        &runtime_dir,
        &socket_path,
...
fn wait_agent_status_exits_immediately_when_status_already_matches() {
    ...
    let reported = send_request(
        &socket_path,
        &format!(
            r#"{{"id":"req_cli_immediate_2","method":"pane.report_agent","params":{{"pane_id":"{}","source":"shuvr:pi","agent":"pi","state":"idle"}}}}"#,
            pane_id
        ),
    );
