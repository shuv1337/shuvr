...
- `shuvr session stop` no longer fails on zero-duration socket timeouts when the stop deadline is nearly exhausted.
...
### Added
- Added native agent session restore behind `[session] resume_agents_on_restore`, allowing supported Pi, Claude Code, Codex, OpenCode, and Hermes panes with current official integrations to restart into their previous agent conversation after a Shuvr server restart. (#233)
...
- Added `ui.mobile_width_threshold` to configure the terminal width at which Shuvr switches to the mobile single-column layout. (#317)
- Added `--handoff` for `shuvr update` and `shuvr --remote` to opt into live server handoff for supported running servers. Plain update and remote attach use the normal restart/stop flow by default.
- Added `pane.report_metadata` and `shuvr pane report-metadata` so user hooks can customize pane titles, displayed agent names, compact status labels, and visible state labels without taking over integration-owned lifecycle or session state. (#36)
...
- Added Ctrl-click URL opening inside panes for OSC 8 hyperlinks and visible `http://` or `https://` URLs when the host terminal sends the modified click to Shuvr. (#307)
- Added Qoder CLI detection, terminal state heuristics, and `shuvr integration install qodercli` hook support. (#308, #309, thanks @wayneleelwc)

### Fixed
- Remote bootstrap now downloads exact-version release assets for Homebrew and Nix clients instead of copying package-manager-managed local binaries into `~/.local/bin/shuvr`.
...
- `shuvr session stop` is more reliable when the server closes the socket early or stops without sending a full response.
...
- Linux sound alerts no longer fall back to `aplay` for mp3 files, preventing static noise on systems without `paplay`. Shuvr now tries mp3-capable players such as `pw-play`, `ffplay`, `mpg123`, and `mpv` instead. (#290)

## [0.6.2] - 2026-05-23

### Added
- Added optional Nix flake support for building, running, installing, and developing Shuvr with Nix. (#208, #221, #264)
- Added `terminal.new_cwd` to choose whether new panes, tabs, and workspaces follow the source pane/workspace, start in `$HOME`, use Shuvr's process directory, or use a fixed path.
- Added `shuvr integration install omp` for OMP's `.omp` extension directory. The extension reports OMP pane state through Shuvr's socket API without relying on native `omp` process detection.
- Added CLI and socket API support for Git worktrees with `shuvr worktree list/create/open/remove`, optional worktree provenance on workspace responses, and client/server protocol version 10.
...
- Added `--remote-keybindings local|server` for `shuvr --remote`. Remote attach now uses the launching client's local keybindings by default without copying config files to the remote host; use `--remote-keybindings server` to keep the remote server's keybindings. The client/server protocol is now version 9.
...
### Fixed
- Pane apps that query OSC 10/11 default foreground/background colors now receive the host terminal colors, so OpenCode and similar TUIs can detect light terminal themes inside Shuvr. (#253)
...
- `shuvr --remote` now offers to restart the remote server after installing or replacing a remote binary, or when the running server version differs, even if the client/server protocol is still compatible.
...
- Added `shuvr config reset-keys` to back up `config.toml` and remove custom keybindings so built-in v2 defaults apply on restart or config reload. (#154)
- Added an integrations tab in settings and first-run onboarding so users can install recommended agent integrations from inside Shuvr.
...
- Added `terminal.default_shell` to choose the executable used for new interactive panes. When unset, Shuvr still falls back to `$SHELL`, then `/bin/sh`. (#196)
...
- Running `shuvr server` directly now prints socket and log paths and explains that normal TUI users should run `shuvr`.
- Kitty graphics virtual Unicode placeholders now render image placements instead of leaving placeholder cells behind. (#136)
- Clipboard image reads are now capped to Shuvr's image payload limit, preventing oversized local clipboard images from being read into memory.
- The install script now reads Shuvr's public latest-release manifest, so fresh installs use the same binary URLs as `shuvr update`.
...
- Remote clients now bridge local clipboard images into the remote pane by staging them as temporary image files and pasting the remote path, so Claude Code image paste works over `shuvr --remote`. (#205)
...
### Fixed
- The Claude Code integration no longer reports successful or failed post-tool hooks as `working`, and installing the updated integration removes Shuvr's deprecated post-tool hook entries from existing Claude settings. (#198)
...
### Added
- Added the `terminal` built-in theme, which uses the host terminal's ANSI palette for Shuvr UI colors. (#140, #146, thanks @babymastodon)
...
### Fixed
- Running the internal `shuvr client` command from inside Shuvr now respects the nested-launch guard, and the command is no longer advertised in root help. (#187)
- The Shuvr agent skill now refuses to claim pane ownership unless it is running inside Shuvr. (#152)
...
- Shuvr now preserves `session.json` symlinks when saving persistent session state. (#139, #147, thanks @cloudmanic)
...
- Remote attach now uses a matching `shuvr` already available on the remote `PATH` before installing a new copy. (#170)
...
- Added direct terminal attach with `shuvr agent attach <target>` and `shuvr terminal attach <terminal_id>`.
...
- Shuvr now redraws immediately when the outer terminal regains focus.
...
### Fixed
- PageUp and PageDown now scroll Shuvr pane scrollback for normal panes while still forwarding keys to full-screen or mouse-reporting apps.
- Enhanced tilde key sequences now parse correctly, improving compatibility with terminals that emit them.
- `shuvr integration install codex` now enables the current Codex `[features] hooks = true` flag and migrates the deprecated top-level `codex_hooks` flag.

### Breaking Changes
- `advanced.allow_nested` has moved to `experimental.allow_nested`; update configs that allow nested Shuvr launches.
...
### Added
- Added manual pane labels through `shuvr pane rename`, the `pane.rename` socket API, an optional `keys.rename_pane` binding, and the right-click pane menu.
...
- Added `shuvr integration status [--outdated-only]` so installed agent integrations can be checked for legacy or outdated versions.
...
### Added
- Added ANSI-formatted pane reads to the CLI and socket API with `shuvr pane read --format ansi` / `--ansi`, preserving colors and styles for visible and recent pane output.
...
- `shuvr update` now downloads and prepares the new binary before stopping a running server, reducing the chance of interrupting an active session when download or install preparation fails.

## [0.5.6] - 2026-05-09

### Added
- Added the `vesper` built-in theme. (#71, thanks @nexxeln)
- Added `shuvr --remote <ssh-target>`, so you can use Shuvr as a thin client for remote servers without SSHing in first. Shuvr connects over SSH, bootstraps a matching remote `shuvr` binary when needed, starts the remote server automatically, and streams an efficient terminal view back to your local terminal.
...
### Fixed
- OSC 8 hyperlinks emitted inside panes now remain clickable after Shuvr renders them, including titled markdown-style links.
...
### Added
- Added a mobile layout for narrow terminals, making it practical to SSH into your machine and run shuvr from your phone.
...
### Fixed
- Visible active-tab panes that finish while the outer terminal is unfocused are now marked as seen when you return to shuvr, preventing stale done/attention indicators.
...
### Added
- Added named persistent sessions, so you can keep separate shuvr environments for different projects or contexts while sharing the same global config. See the docs for the full session CLI. (#57, thanks @fbettag)
- Added `shuvr status`, `shuvr status server`, and `shuvr status client` to inspect the local client, running server, protocol compatibility, socket path, and whether a restart is needed.

### Changed
- Focused panes can now still alert you through terminal notifications when the shuvr terminal window is unfocused, so active work does not go quiet just because you switched to another app.
...
- `shuvr integration install` and `shuvr integration uninstall` now work without requiring a running shuvr server.
...
### Added
- Config can now be reloaded in the running app/server from the global menu or with `shuvr server reload-config`, applying safe live settings without restarting the persistent server.
...
### Added
- Toast notifications can now be delivered through the outer terminal as desktop notifications. Configure this with `ui.toast.delivery = "terminal"`; see the [configuration docs](https://github.com/shuv1337/shuvr/docs/configuration/) for details.
- Shuvr now writes separate capped support logs for app, client, and server modes, making persistent-session issue reports easier to diagnose without unbounded log growth.
...
- In-app toast delivery now stays inside shuvr instead of also forwarding a terminal/desktop notification.

## [0.5.0] - 2026-04-21

### Breaking Changes Please Read
- shuvr now defaults to a persistent server/client session model. running `shuvr` starts or reattaches to a background session server instead of launching the old single-process UI.
- quitting the UI in default mode now detaches the current client and leaves the shared session running. use `shuvr server stop` to stop the background server explicitly.
- the old monolithic behavior is still available as an escape hatch with `shuvr --no-session`.

### Added
- Persistent sessions are now the default product behavior. You can detach and reattach without stopping pane processes.
- Added the thin client and headless server as first-class product components, including auto-detect launch, explicit `shuvr client`, and `shuvr server stop`.
...
### Breaking Changes Please Read
- The update flow changes in `0.4.11`. Shuvr no longer installs updates silently in the background. Starting with this release, shuvr only checks for updates and shows them in the UI. To install a new release, quit shuvr and then run `shuvr update` manually in your shell.
- This prepares the upcoming `0.5.0` persistence release. Shuvr is moving from the old single-binary update model toward a persistent server/client session model, so your workspace can keep running while clients attach, detach, and reconnect.
- The reason for this change is upgrade safety. Shuvr needs to stop the old running process cleanly before the new client/server model takes over, so manual update avoids mixed-version states during the transition.

### Added
- Hook-reported agent state can now use custom agent labels, so integrations are no longer limited to shuvr’s built-in agent names. Custom labels now flow through pane/workspace UI and the socket API anywhere agent names are shown.

## [0.4.10] - 2026-04-14

### Added
- Prefix mode now supports custom command keybindings via `[[keys.command]]`, so you can launch detached shell helpers or open temporary overlay panes from inside shuvr using the active workspace, tab, pane, and cwd context.
...
### Fixed
- Droid's primary-screen redraws no longer erase pane scrollback inside shuvr, while normal scrollback-clear behavior is preserved elsewhere.
...
### Added
- Themes can now set `panel_bg = "reset"` to let shuvr’s panel chrome inherit the host terminal background instead of painting an opaque panel fill. This also accepts the aliases `default`, `none`, and `transparent`.
...
### Added
- `shuvr workspace create` and `shuvr tab create` now support `--label`, so scripts and agents can name new workspaces and tabs immediately instead of creating them first and renaming them afterward.
- The global menu now includes a manual **reload keybinds** action, so you can apply `config.toml` keybinding changes without restarting shuvr.
- The socket API and CLI now expose a `done` agent status, including `shuvr wait agent-status --status done`, so automation can distinguish finished agent runs from panes that are merely idle.

### Changed
- Session state is now saved automatically with a debounce while you work, so recent workspace, tab, pane, and sidebar changes are preserved more reliably even if shuvr exits unexpectedly.
...
- `shuvr pane run ...` now uses the bracketed-paste-aware input path, improving compatibility with shells and terminal apps that expect pasted command text to arrive atomically.
...
- `shuvr pane run ...` now submits the full command atomically in one request, fixing cases where scripted commands did not reliably execute because the final Enter was sent separately.
...
### Major Changes
- Shuvr now uses a Ghostty-backed terminal engine as its pane runtime.
...
- Debug builds now use separate runtime/config paths from normal releases, which avoids local development sessions colliding with your main shuvr install.

### Fixed
- Starting a second shuvr instance against an active socket now fails fast with a clear error instead of clobbering the running session.
...
- Added first-class tab support to the local socket API and CLI wrappers, including `shuvr tab ...` commands and tab ids like `1:2` alongside workspace-scoped pane ids.
...
- Added a post-update release-notes screen so shuvr can explain what changed after an update is installed.
...
### Changed
- Mouse wheel handling now follows the tmux/Ghostty model more closely: fullscreen apps receive wheel input when they own scrolling, while shuvr keeps host scrollback for panes that are behaving like a normal terminal transcript.
- Pane scrollbars now only appear when shuvr has real host scrollback for that pane, instead of implying a host-managed scroll position for app-owned scrolling.

### Fixed
- Fixed Codex and pi panes becoming unscrollable in shuvr by preserving recoverable host history for top-anchored normal-screen output, without relying on alternate-screen scrollback retention.
- Fixed pane wheel routing so apps using mouse reporting or alternate-scroll behavior can receive scroll input directly instead of having shuvr always intercept it.
...
- Fixed alternate-screen scrollback handling so full-screen terminal apps can preserve recoverable history inside shuvr panes instead of losing rows that scroll off.
- Fixed Codex in shuvr panes losing transcript/history while running in alternate screen, so past output remains scrollable instead of disappearing as the session grows.
...
### Added
- Shuvr now checks for updates at startup and periodically while it stays open, so long-running sessions can still discover new releases without a restart cycle.
...
### Added
- Added a local Unix socket API for controlling running shuvr sessions, including workspace and pane management, pane reads, text/key input, pane splitting, and output waits.
...
- Added CLI wrappers on top of the socket API with `shuvr workspace ...`, `shuvr pane ...`, and `shuvr wait ...`, using compact public ids like `1` and `1-2` for scripting and agent orchestration.
- Added a settings popup with mouse support for changing themes, sound alerts, and toast notifications from inside shuvr.
...
- Shuvr now blocks nested launches by default when started inside a shuvr-managed pane; set `advanced.allow_nested = true` to opt back in.
...
### Documentation
- Refreshed the README, socket API reference, and reusable agent skill docs to better explain shuvr's agent multiplexer model and integration surface.
...
- Added configurable keybindings for all navigate mode actions: new workspace, rename workspace, close workspace, resize mode, and toggle sidebar. See the [configuration docs](https://github.com/shuv1337/shuvr/docs/configuration/) for the full key reference.
...
### Fixed
- Improved foreground job detection on Linux and macOS so shuvr can recognize agents that run through wrapper processes or generic runtimes, including cases like Codex running under `node`.
