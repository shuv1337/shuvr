...
// SHUVR_INTEGRATION_ID=omp
// SHUVR_INTEGRATION_VERSION=2
// @ts-nocheck

import { createConnection } from "node:net";

const SHUVR_ENV = process.env.SHUVR_ENV;
const socketPath = process.env.SHUVR_SOCKET_PATH;
const paneId = process.env.SHUVR_PANE_ID;
const source = "shuvr:omp";

function enabled() {
  return SHUVR_ENV === "1" && !!socketPath && !!paneId;
}
...
const idleDebounceMs = parseDurationEnv("SHUVR_OMP_IDLE_DEBOUNCE_MS", 250);
const retryGraceMs = parseDurationEnv("SHUVR_OMP_RETRY_GRACE_MS", 2500);
...
export default function (pi) {
  ...
  pi.events.on("shuvr:blocked", (data) => {
    if (!data?.active) {
