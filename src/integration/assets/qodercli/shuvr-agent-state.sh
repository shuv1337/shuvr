...
# SHUVR_INTEGRATION_ID=qodercli
# SHUVR_INTEGRATION_VERSION=1
#
# Reports qodercli agent state changes to shuvr. Registered as a Command hook
# in ~/.qoder/settings.json by `shuvr integration install qodercli` and
...
# `assets/claude/shuvr-agent-state.sh` already consumes claude code's stdin
# payload. No environment variable is consulted for the event identity.

set -eu

action="${1:-}"
hook_input_file="$(mktemp "${TMPDIR:-/tmp}/shuvr-qodercli-hook.XXXXXX")" || exit 0
...
[ "${SHUVR_ENV:-}" = "1" ] || exit 0
[ -n "${SHUVR_SOCKET_PATH:-}" ] || exit 0
[ -n "${SHUVR_PANE_ID:-}" ] || exit 0
command -v python3 >/dev/null 2>&1 || exit 0

SHUVR_ACTION="$action" SHUVR_HOOK_INPUT_FILE="$hook_input_file" python3 - <<'PY'
import json
import os
import random
import socket
...
source = "shuvr:qodercli"
action = os.environ.get("SHUVR_ACTION", "")
pane_id = os.environ.get("SHUVR_PANE_ID")
socket_path = os.environ.get("SHUVR_SOCKET_PATH")
hook_input_file = os.environ.get("SHUVR_HOOK_INPUT_FILE")
