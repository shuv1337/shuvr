            ...
            if let Some(workspace) = self.state.workspaces.get(ws_idx) {
                ...
                if let Some(pane_id) = workspace.focused_pane_id() {
                    ...
                    if let Some(pane_cwd) = workspace.active_tab().and_then(|tab| {
                        ...
                        env.push((
                            "SHUVR_ACTIVE_PANE_CWD".to_string(),
...
fn unique_scrollback_path(attempt: u32) -> std::path::PathBuf {
    ...
    std::env::temp_dir().join(format!(
        "shuvr-scrollback-{}-{nanos}-{attempt}.txt",
        std::process::id()
    ))
}
...
mod tests {
    ...
    fn rename_workspace_prefills_live_terminal_cwd_label() {
        ...
        state.workspaces[0].identity_cwd = "/__shuvr_original__".into();
        state.terminals.insert(
            terminal_id.clone(),
            TerminalState::new(terminal_id, "/__shuvr_projects__".into()),
        );
        state.keybinds.rename_workspace = crate::config::ActionKeybinds::prefix("g");
        ...
        assert_eq!(state.mode, Mode::RenameWorkspace);
        assert_eq!(state.name_input, "__shuvr_projects__");
        assert_eq!(state.workspaces[0].display_name(), "__shuvr_original__");
    }
...
"#,
    ...
    async fn custom_command_runs_from_prefix_key_in_navigate_mode() {
        ...
        let command = format!(
            "printf '%s\\n%s\\n%s\\n' \"$SHUVR_ACTIVE_WORKSPACE_ID\" \"$SHUVR_ACTIVE_TAB_ID\" \"$SHUVR_ACTIVE_PANE_ID\" > '{}'",
            output_path.display()
        );
