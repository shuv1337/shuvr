...
impl App {
    ...
    pub(super) fn handle_worktree_remove(
        ...
        let Some(space) = self
            ...
            return encode_error(
                id,
                "not_linked_worktree",
                "workspace is not a Shuvr-managed worktree checkout",
            );
        };
    ...
    fn resolve_worktree_source(
        ...
        if let Some(cwd) = cwd {
            ...
            let space = crate::workspace::git_space_metadata(&path).ok_or_else(|| {
                ApiFailure::new(
                    "not_git_worktree",
                    "Shuvr worktree actions require a path inside a Git work tree",
                )
            })?;
            if space.is_linked_worktree {
                return Err(ApiFailure::new(
    ...
    fn worktree_source_from_workspace(&self, ws_idx: usize) -> Result<WorktreeSource, ApiFailure> {
        ...
        let Some(space) = git_space else {
            return Err(ApiFailure::new(
                "not_git_worktree",
                "Shuvr worktree actions require a workspace inside a Git work tree",
            ));
        };
        if space.is_linked_worktree {
            return Err(ApiFailure::new(
