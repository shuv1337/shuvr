...
mod tests {
    ...
    fn restored_worktree_space_membership_drops_missing_checkout() {
        let missing =
            std::env::temp_dir().join(format!("shuvr-missing-worktree-{}", std::process::id()));
    ...
    fn restore_rehydrates_agent_session_metadata() {
        ...
        assert_eq!(preserved.source, "shuvr:hermes");
    ...
    async fn restore_carries_persisted_agent_session_metadata() {
        ...
        assert_eq!(session.source, "shuvr:opencode");
