...
/// Serializable snapshot of the entire shuvr session.
...
mod tests {
    ...
    fn session_fixture(name: &str) -> &'static str {
        match name {
            "current-shuvr" => {
                include_str!("../../tests/fixtures/session/current-shuvr-session.json")
            }
            "current-shuvr-dev" => {
                include_str!("../../tests/fixtures/session/current-shuvr-dev-session.json")
            }
            "legacy-pre-tabs-v2" => {
    ...
    fn round_trip_full_workspace_snapshot() {
        ...
        panes.insert(
            0,
            PaneSnapshot {
                cwd: PathBuf::from("/home/can/Projects/shuvr"),
                label: None,
                agent_name: None,
                agent_session: None,
                launch_argv: None,
        ...
        let snap = SessionSnapshot {
            workspaces: vec![WorkspaceSnapshot {
                ...
                identity_cwd: PathBuf::from("/home/can/Projects/shuvr"),
                worktree_space: None,
        ...
        assert_eq!(
            ...
            PathBuf::from("/home/can/Projects/shuvr")
        );
        assert_eq!(
    ...
    fn current_session_fixture_parses() {
        let snap = parse_snapshot(session_fixture("current-shuvr")).unwrap();
    ...
    fn current_dev_session_fixture_parses_additive_fields() {
        let snap = parse_snapshot(session_fixture("current-shuvr-dev")).unwrap();
    ...
    fn legacy_workspace_snapshot_migrates_to_single_tab() {
        ...
        assert_eq!(ws.tabs[0].panes[&1].cwd, PathBuf::from("/tmp/shuvr"));
    }
    ...
    fn capture_contract_tracks_worktree_space_membership() {
        ...
        state.workspaces[0].worktree_space = Some(crate::workspace::WorktreeSpaceMembership {
            key: "repo-key".into(),
            label: "shuvr".into(),
            repo_root: PathBuf::from("/repo/shuvr"),
            checkout_path: PathBuf::from("/repo/shuvr/worktree-a"),
    ...
    fn capture_contract_tracks_workspace_identity_and_pane_cwds() {
        ...
        let second_terminal_id = state.workspaces[0].tabs[0].panes[&second]
            .attached_terminal_id
            .clone();
        state.terminals.get_mut(&second_terminal_id).unwrap().cwd = PathBuf::from("/tmp/shuvr");
        ...
        assert_eq!(tab.panes[&second.raw()].cwd, PathBuf::from("/tmp/shuvr"));
    }
    ...
    fn capture_contract_tracks_hook_authority_agent_session() {
        ...
        assert_eq!(agent_session.source, "shuvr:pi");
    ...
    fn capture_contract_preserves_restored_agent_session() {
        ...
        assert_eq!(agent_session.source, "shuvr:opencode");
    ...
    fn restore_falls_back_to_home_when_cwd_missing() {
        ...
        panes.insert(
            0,
            PaneSnapshot {
                cwd: PathBuf::from("/tmp/this-directory-does-not-exist-for-shuvr-test"),
                label: None,
                agent_name: None,
                agent_session: None,
                launch_argv: None,
        ...
        assert_eq!(
            restored.workspaces[0].tabs[0].panes[&0].cwd,
            PathBuf::from("/tmp/this-directory-does-not-exist-for-shuvr-test")
        );
    }
}
