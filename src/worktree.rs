...
";
    ...
    fn expand_tilde_path_uses_home_when_available() {
        ...
        assert_eq!(
            expand_tilde_path("~/.shuvr/worktrees"),
            PathBuf::from("/home/me/.shuvr/worktrees")
        );
        assert_eq!(
    ...
    fn default_checkout_path_appends_repo_and_branch_slug() {
        assert_eq!(
            default_checkout_path(
                Path::new("/home/me/.shuvr/worktrees"),
                "shuvr",
                "worktree/brave-river",
            ),
            PathBuf::from("/home/me/.shuvr/worktrees/shuvr/worktree-brave-river")
        );
    }
    ...
    fn dirty_remove_error_detection_matches_git_force_hint() {
        assert!(is_dirty_worktree_remove_error(
            "fatal: '/w/shuvr' contains modified or untracked files, use --force to delete it"
        ));
        assert!(!is_dirty_worktree_remove_error(
            "fatal: '/w/shuvr' is a missing but already registered worktree"
        ));
        assert!(!is_dirty_worktree_remove_error(
            "fatal: '/w/shuvr' contains a locked worktree, use --force only if you know why"
        ));
    }
    ...
    fn worktree_add_command_creates_new_branch_from_base() {
        let command = build_worktree_add_new_branch_command(
            Path::new("/repo/shuvr"),
            Path::new("/w/shuvr/worktree-brave-river"),
            "worktree/brave-river",
            "HEAD",
        );
        assert_eq!(command.program, "git");
