//! Auto-detect launch behavior for the `shuvr` command.
//!
//! When the user runs `shuvr` with no subcommand:
...
/// Checks whether a shuvr server is currently listening on the client socket.
///
...
/// Checks whether a shuvr server is listening at a specific socket path.
...
fn validate_running_server_compatibility() -> io::Result<()> {
    ...
    let Some(status) = read_server_status()? else {
        return Err(io::Error::other(format!(
            "a shuvr server is listening, but its status API is unavailable. Try `{stop_command}`; if that fails, stop the old server process manually, then run `{attach_command}` again."
        )));
    };
    ...
    Err(io::Error::other(format!(
        "shuvr server is running from v{} / protocol {}, but this client is v{} / protocol {}.\nStop the old server with `{stop_command}`, then run `{attach_command}` again.",
...
/// Spawns the shuvr server as a background daemon process.
///
...
/// - Inherits relevant environment variables (`XDG_CONFIG_HOME`, `SHUVR_SESSION`,
...
pub fn spawn_server_daemon() -> io::Result<u32> {
    ...
    let child = command.spawn().map_err(|err: io::Error| {
        io::Error::new(err.kind(), format!("failed to spawn shuvr server: {err}"))
    })?;
...
///
/// This is the entry point called from `main.rs` when the user runs `shuvr`
...
mod tests {
    ...
    fn server_daemon_command_clears_socket_overrides_for_explicit_session() {
        ...
        std::env::set_var("SHUVR_CLIENT_SOCKET_PATH", "/tmp/inherited-client.sock");
        ...
        let command = build_server_daemon_command(PathBuf::from("/tmp/shuvr-test"));
        ...
        assert!(envs.iter().any(|(key, value)| {
            *key == OsStr::new("SHUVR_CLIENT_SOCKET_PATH") && value.is_none()
        }));
        std::env::remove_var(crate::api::SOCKET_PATH_ENV_VAR);
        std::env::remove_var("SHUVR_CLIENT_SOCKET_PATH");
        std::env::remove_var(crate::session::SESSION_ENV_VAR);
        crate::session::clear_explicit_session_for_test();
    }
    ...
    fn validate_running_server_compatibility_names_session_commands_for_protocol_mismatch() {
        ...
        let _ = handle.join();
        assert!(
            message.contains("Stop the old server with `shuvr session stop work`"),
            "unexpected error: {message}"
        );
        assert!(
            message.contains("then run `shuvr session attach work` again"),
            "unexpected error: {message}"
        );
        std::env::remove_var(crate::api::SOCKET_PATH_ENV_VAR);
        std::env::remove_var(crate::session::SESSION_ENV_VAR);
