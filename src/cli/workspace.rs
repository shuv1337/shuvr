...
fn workspace_list(args: &[String]) -> std::io::Result<i32> {
    if !args.is_empty() {
        eprintln!("usage: shuvr workspace list");
        return Ok(2);
    }
...
fn workspace_get(args: &[String]) -> std::io::Result<i32> {
    let Some(raw_workspace_id) = args.first() else {
        eprintln!("usage: shuvr workspace get <workspace_id>");
        return Ok(2);
    };
    if args.len() != 1 {
        eprintln!("usage: shuvr workspace get <workspace_id>");
...
fn workspace_focus(args: &[String]) -> std::io::Result<i32> {
    let Some(raw_workspace_id) = args.first() else {
        eprintln!("usage: shuvr workspace focus <workspace_id>");
        return Ok(2);
    };
    if args.len() != 1 {
        eprintln!("usage: shuvr workspace focus <workspace_id>");
...
fn workspace_rename(args: &[String]) -> std::io::Result<i32> {
    if args.len() < 2 {
        eprintln!("usage: shuvr workspace rename <workspace_id> <label>");
        return Ok(2);
    }
...
fn workspace_close(args: &[String]) -> std::io::Result<i32> {
    let Some(raw_workspace_id) = args.first() else {
        eprintln!("usage: shuvr workspace close <workspace_id>");
        return Ok(2);
    };
    if args.len() != 1 {
        eprintln!("usage: shuvr workspace close <workspace_id>");
...
fn print_workspace_help() {
    eprintln!("shuvr workspace commands:");
    eprintln!("  shuvr workspace list");
    eprintln!("  shuvr workspace create [--cwd PATH] [--label TEXT] [--focus] [--no-focus]");
    eprintln!("  shuvr workspace get <workspace_id>");
    eprintln!("  shuvr workspace focus <workspace_id>");
    eprintln!("  shuvr workspace rename <workspace_id> <label>");
    eprintln!("  shuvr workspace close <workspace_id>");
}
