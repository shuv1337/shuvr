...
fn parse_status_args(args: &[String]) -> Option<(StatusScope, bool)> {
    match args.first().map(|arg| arg.as_str()) {
        ...
        Some("server") => {
            parse_status_scope_args(args, StatusScope::Server, "shuvr status server [--json]")
        }
        Some("client") => {
            parse_status_scope_args(args, StatusScope::Client, "shuvr status client [--json]")
        }
        Some("help" | "--help" | "-h") => {
            if args.len() > 1 {
                print_status_help();
...
fn print_status_help() {
    eprintln!("shuvr status commands:");
    eprintln!("  shuvr status [--json]         show local client and running server status");
    eprintln!("  shuvr status server [--json]  show running server status");
    eprintln!("  shuvr status client [--json]  show local client binary status");
}
