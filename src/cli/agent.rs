...
fn agent_start(args: &[String]) -> std::io::Result<i32> {
    let Some(name) = args.first() else {
        eprintln!("usage: shuvr agent start <name> [--cwd PATH] [--workspace ID] [--tab ID] [--split right|down] [--focus|--no-focus] -- <argv...>");
        return Ok(2);
    };
...
fn agent_list(args: &[String]) -> std::io::Result<i32> {
    if !args.is_empty() {
        eprintln!("usage: shuvr agent list");
        return Ok(2);
    }
...
fn agent_get(args: &[String]) -> std::io::Result<i32> {
    let Some(target) = args.first() else {
        eprintln!("usage: shuvr agent get <target>");
        return Ok(2);
    };
    if args.len() != 1 {
        eprintln!("usage: shuvr agent get <target>");
...
fn agent_focus(args: &[String]) -> std::io::Result<i32> {
    let Some(target) = args.first() else {
        eprintln!("usage: shuvr agent focus <target>");
        return Ok(2);
    };
    if args.len() != 1 {
        eprintln!("usage: shuvr agent focus <target>");
...
fn agent_attach(args: &[String]) -> std::io::Result<i32> {
    let (target, takeover) =
        match super::parse_attach_target(args, "usage: shuvr agent attach <target> [--takeover]") {
            Ok(parsed) => parsed,
...
fn agent_wait(args: &[String]) -> std::io::Result<i32> {
    let Some(target) = args.first() else {
        eprintln!("usage: shuvr agent wait <target> --status <idle|working|blocked|unknown> [--timeout MS]");
        return Ok(2);
    };
...
fn agent_rename(args: &[String]) -> std::io::Result<i32> {
    let Some(target) = args.first() else {
        eprintln!("usage: shuvr agent rename <target> <name>|--clear");
        return Ok(2);
    };
    if args.len() < 2 {
        eprintln!("usage: shuvr agent rename <target> <name>|--clear");
...
fn agent_send(args: &[String]) -> std::io::Result<i32> {
    if args.len() < 2 {
        eprintln!("usage: shuvr agent send <target> <text>");
        return Ok(2);
    }
...
fn agent_read(args: &[String]) -> std::io::Result<i32> {
    let Some(target) = args.first() else {
        eprintln!("usage: shuvr agent read <target> [--source visible|recent|recent-unwrapped] [--lines N] [--format text|ansi] [--ansi]");
        return Ok(2);
    };
...
fn print_agent_help() {
    eprintln!("shuvr agent commands:");
    eprintln!("  shuvr agent list");
    eprintln!("  shuvr agent get <target>");
    eprintln!("  shuvr agent read <target> [--source visible|recent|recent-unwrapped] [--lines N] [--format text|ansi] [--ansi]");
    eprintln!("  shuvr agent send <target> <text>");
    eprintln!("  shuvr agent rename <target> <name>|--clear");
    eprintln!("  shuvr agent focus <target>");
    eprintln!("  shuvr agent wait <target> --status <idle|working|blocked|unknown> [--timeout MS]");
    eprintln!("  shuvr agent attach <target> [--takeover]");
    eprintln!("  shuvr agent start <name> [--cwd PATH] [--workspace ID] [--tab ID] [--split right|down] [--focus|--no-focus] -- <argv...>");
