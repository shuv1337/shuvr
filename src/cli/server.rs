...
fn server_stop(args: &[String]) -> std::io::Result<i32> {
    if !args.is_empty() {
        eprintln!("usage: shuvr server stop");
        return Ok(2);
    }
...
fn server_reload_config(args: &[String]) -> std::io::Result<i32> {
    if !args.is_empty() {
        eprintln!("usage: shuvr server reload-config");
        return Ok(2);
    }
...
fn server_live_handoff(args: &[String]) -> std::io::Result<i32> {
    let Some(params) = parse_live_handoff_params(args) else {
        eprintln!(
            "usage: shuvr server live-handoff [--import-exe <path>] [--expected-protocol <n>] [--expected-version <version>]"
        );
        return Ok(2);
    };
...
fn print_server_help() {
    eprintln!("shuvr server commands:");
    eprintln!("  shuvr server                run as headless server");
    eprintln!("  shuvr server stop           stop the running server via the API socket");
    eprintln!("  shuvr server live-handoff   hand off live panes to a new local server");
    eprintln!("  shuvr server reload-config  reload config.toml in the running server");
}
...
mod tests {
    ...
    fn live_handoff_params_parse_remote_update_fields() {
        let args = vec![
            ...
            "/home/me/.local/bin/shuvr".to_string(),
